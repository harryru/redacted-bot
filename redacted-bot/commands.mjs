...
const listCommands = (args, command) => {
    ...
    if (checkValidSyntax(args, command, 0)) {
        args.channel.send("!r <command> -- commands, invite, play <song/playlist>, skip, previous, loop, playing, pause, resume, queue, clear, shuffle, usercount, roleinfo, post <content>, purge <#>, image, avatar");
        reactComplete(args);
    }
